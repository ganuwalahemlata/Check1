...
(function(g){
    ...
    messages:function(b){
        for(name in b)this.addMessage(name,b[name]);return this
        },
...
bindValidationEvent:function(b,c){
    var d=
    this,h=this.options,j=this.dataNames,l=c.data(j.events).split(" ");
...
jQuery.ketchup.validation("required",thisIsRequired,function(g,b,c){
    g=b.attr("type").toLowerCase();
    return g=="checkbox"||g=="radio"?b.attr("checked")==true:c.length!=0
    }).validation("minlength",minLength + " {arg1}.",function(g,b,c,d){
    return c.length>=+d
    }).validation("maxlength",maxLength + " {arg1}.",function(g,b,c,d){
    return c.length<=+d
    }).validation("rangelength",lengthBetween1 + " {arg1} " + andWord + " {arg2}.",function(g,
    b,c,d,h){
    return c.length>=d&&c.length<=h
    }).validation("min",mustBeAtLeast + " {arg1}.",function(g,b,c,d){
    return this.isNumber(c)&&+c>=+d
    }).validation("max",canNotGreaterThan + " {arg1}.",function(g,b,c,d){
    return this.isNumber(c)&&+c<=+d
    }).validation("range",mustBeBetween + " {arg1} " + andWord + " {arg2}.",function(g,b,c,d,h){
    return this.isNumber(c)&&+c>=+d&&+c<=+h
    }).validation("number",mustBeANumber,function(g,b,c){
    return this.isNumber(c)
    }).validation("digits",mustBeADigits,function(g,b,c){
    return/^\d+$/.test(c)
    }).validation("email",
    validEmail,function(g,b,c){
        return this.isEmail(c)
        }).validation("url",mustBeAValidURL,function(g,b,c){
    return this.isUrl(c)
    }).validation("username",mustBeAValidUsername,function(g,b,c){
    return this.isUsername(c)
    }).validation("match",mustBe + " {arg1}.",function(g,b,c,d){
    return b.val()==d
    }).validation("contain",mustContain + " {arg1}",function(g,b,c,d){
    return this.contains(c,d)
    }).validation("date",mustBeAValidDate,function(g,b,c){
    return this.isDate(c)
    }).validation("minselect",isSelected,function(g,b,c,d){
        return d<=this.inputsWithName(g,b).filter(":checked").length
        },function(g,b){
        this.bindBrothers(g,b)
        }).validation("maxselect",selectNotMoreThan + " {arg1} " + varCheckboxes + ".",function(g,b,c,d){
    return d>=this.inputsWithName(g,b).filter(":checked").length
    },function(g,b){
    this.bindBrothers(g,b)
    }).validation("rangeselect",selectBetween + " {arg1} " + andWord + " {arg2} " + varCheckboxes + ".",function(g,b,c,d,h){
...
jQuery.ketchup.helper("isNumber",function(g){
    ...
    return/^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|[\uE000-\uF8FF]|\/|\?)*)?(\#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@)|\/|\?)*)?$/i.test(g)}).helper("isUsername",
function(g){
